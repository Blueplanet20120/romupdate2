...
function action_start_update()
    ...
    if action == "update" then
        ...
        local script
        if mode == "nokeep" then
            script = uci:get("romupdate", "general", "update_script_nokeep") or "/usr/share/Lenyu-no-auto.sh"
        else
            script = uci:get("romupdate", "general", "update_script_keep") or "/usr/share/Lenyu-auto.sh"
        end
